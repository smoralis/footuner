...
images = {
	path: fb.ProfilePath + "themes\\footuner\\js\\samples\\js-smooth\\images\\",
	glass_reflect: null,
	loading_angle: 0,
	loading_draw: null,
	noart: null,
...
ppt = {
	...
	tf_albumartist: fb.TitleFormat("$if3([%stream_tunein_name%],[%stream_crb_name%],[%stream_ffprobe_name%],[$info(@)],[%path%])"),
	tf_groupkey_genre: fb.TitleFormat("$if2($meta(genre,0),Other)"),
	tf_groupkey_artist: fb.TitleFormat("$if2($meta(artist,0),Unknow Artist)"),
	tf_groupkey_album: fb.TitleFormat("$if3([%stream_tunein_name%],[%stream_crb_name%],[%stream_ffprobe_name%],[$info(@)],[%path%])"),
	...
	wallpaperpath: window.GetProperty("_PROPERTY: Default Wallpaper Path", ".\\themes\\footuner\\js\\samples\\js-smooth\\images\\default.png"),
...
oBrowser = function (name) {
	...
	this.draw = function (gr) {
		...
		this.getlimits();
					...
					case 3:
						...
						if (ppt.panelMode == 1) { // panelMode = 1 (Art + bottom labels)
							...
							if (ppt.showAllItem && i == 0 && total > 1) { // aggregate item ( [ALL] )
								try {
									if (ppt.tagMode == 1) {
										...
										gr.GdiDrawText("(" + (total - 1) + " stations)", g_font, txt_color2, ax + Math.round((aw - coverWidth) / 2), (coverTop + 5 + coverWidth + ppt.botTextRowHeight), coverWidth, ppt.botTextRowHeight + ppt.extra_font_size, DT_LEFT | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
									} else {
								...
								try {
									...
									//	if (this.groups[i].tracktype != 3)
											gr.GdiDrawText(arr[0], g_font_bold, txt_color2, ax + Math.round((aw - coverWidth) / 2), (coverTop + 5 + coverWidth + ppt.botTextRowHeight), coverWidth, ppt.botTextRowHeight + ppt.extra_font_size, DT_LEFT | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
									} else {
							...
							if (ppt.showAllItem && i == 0 && total > 1) { // aggregate item ( [ALL] )
								...
								//		if (this.groups[i].tracktype != 3)
											gr.GdiDrawText(arr[0], g_font_bold, txt_color2, ax + 10, (coverTop + 5 + coverWidth + ppt.botTextRowHeight) - ppt.botGridHeight, aw - 20, ppt.botTextRowHeight + ppt.extra_font_size, DT_LEFT | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
									} else {
						...
						if (ppt.showAllItem && i == 0 && total > 1) { // [ALL] item
							...
							case 1: // album
								try {
									...
									gr.GdiDrawText("(" + (total - 1) + " stations)", g_font, txt_color2, ax + coverWidth + this.marginCover * 2, ay + ppt.textLineHeight, aw - coverWidth - this.marginCover * 3, ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
								} catch (e) {}
								break;
							case 2: // artist
								try {
							...
							case 1: // album
								...
								//	if (this.groups[i].tracktype != 3)
										gr.GdiDrawText(arr[0], g_font_bold, txt_color2, ax + coverWidth + this.marginCover * 2, ay + ppt.textLineHeight, aw - coverWidth - this.marginCover * 3, ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
								} catch (e) {}
								break;
							case 2: // artist
								try {
			...
			if (ppt.showHeaderBar) {
				var item_txt = new Array("", "station", "artist", "genre");
	...
	this.settings_context_menu = function (x, y) {
		...
		_menu1.AppendMenuItem(MF_STRING, 111, "([%stream_tunein_name%],[%stream_crb_name%],[%stream_ffprobe_name%],[$info(@)],[%path%])");
