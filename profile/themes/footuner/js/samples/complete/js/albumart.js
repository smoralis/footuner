...
function _albumart (x, y, w, h) {
	...
	this.rbtn_up = (x, y) => {
		...
		panel.m.AppendMenuSeparator();
		panel.m.AppendMenuItem(panel.metadb ? MF_STRING : MF_GRAYED, 1040, 'Open Tunein Webpage');
		panel.m.AppendMenuSeparator();
	}
	...
	this.rbtn_up_done = (idx) => {
		...
		case 1030:
			...
			break;
		case 1040:
			if (panel.tf('%STREAM_TUNEIN_ID%')) _run('https://tunein.com/station/?stationId=' + encodeURIComponent(panel.tf('%STREAM_TUNEIN_ID%')).slice(1));
			break;
		}
	}
	...
	let get_album_art = async (obj) => {
		...
		if (panel.metadb && result.image) {
			...
			let json_file = fb.FoobarPath + 'user-data\\mtags\\' + fb.TitleFormat('%directory%').EvalWithMetadb(panel.metadb) + "\\" + fb.TitleFormat('%filename%').EvalWithMetadb(panel.metadb) + '.info.json';
			if (utils.FileTest(json_file, "e")){
				let json_data = _jsonParse(utils.ReadTextFile(json_file));
				obj.tooltip = "";
				obj.tooltip += json_data.body[0].name + '\n\n';				
				if (json_data.body[0].location) obj.tooltip += "Location/Country : " + json_data.body[0].location + '\n';
				if (json_data.body[0].language) obj.tooltip += "Language : " + json_data.body[0].language + '\n';
				if (json_data.body[0].genre_name) obj.tooltip += "Genre/Tags : " + json_data.body[0].genre_name + '\n';
				if (json_data.body[0].slogan) obj.tooltip += "Slogan : " + json_data.body[0].slogan + '\n\n';
				else obj.tooltip += "\n";
				if (json_data.body[0].description) obj.tooltip += "Description :\n" + json_data.body[0].description + '\n';

			}
			else {
				obj.tooltip = "No Station Info";
			}
		}
		window.Repaint();
	}
