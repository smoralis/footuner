...
function _thumbs() {
    ...
    this.rbtn_up = (x, y) => {
        ...
        panel.m.AppendMenuSeparator();
        panel.m.AppendMenuItem(MF_STRING, 2000, 'Enter Lastfm ApiKey');
		panel.m.AppendMenuSeparator();
    }
    ...
    this.rbtn_up_done = (idx) => {
        ...
        case 2000:
            let key_input = "";
            try {
                key_input = utils.InputBox(window.ID, "ENTER Your Lastfm ApiKey", "Lastfm ApiKey", api_key, true);
                api_key = key_input;
                utils.WriteINI(settings_file, 'lastfm', 'apikey', key_input);
    ...
    this.close_btn = new _sb(chars.close, 0, 0, _scale(12), _scale(12), () => {
            return this.properties.mode.value == 0 && this.overlay;
        }, () => {
            this.enable_overlay(false);
        });
