...
function _text(mode, x, y, w, h) {
	...
	this.rbtn_up = (x, y) => {
		...
		case 'text_reader':
			...
			panel.m.AppendMenuSeparator();
			panel.m.AppendMenuItem(MF_STRING, 1240, 'Log Mode');
			panel.m.CheckMenuItem(1240, this.properties.logmode.enabled);
			panel.m.AppendMenuSeparator();
			break;
		}
	...
	this.rbtn_up_done = (idx) => {
		...
		case 1230:
			...
			break;
		case 1240:
			this.properties.logmode.toggle();
			this.update();
			break;
		case 1999:
			if (_isFile(this.filename)) {
	...
	this.init = () => {
		...
		case 'text_reader':
			...
			this.properties.logmode = new _p('2K3.TEXT.LOGMODE', true);
			this.exts = 'txt|log';
			break;
		}
	}
